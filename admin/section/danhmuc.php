...
if (isset($_POST['themLoaiSanPham'])) {
    ...
    try {
        $obj->themDanhMucSanPham($conn, $ma_danh_muc, $ten_danh_muc);
        $objLSHD->taoLichSuHoatDong($conn, $_SESSION['id_tai_khoan'], "Thêm danh mục " .  $ten_danh_muc);
...
<table class="table table-bordered">
    ...
    <tbody>
        <?php
        $data = $obj->layDanhMucSanPham($conn);
        ...
        foreach ($data as $row) {
            ...
            echo '<td class="text-center remove"><a href="handle/xoa_danh_muc.php?ma_danh_muc=' . $row['ma_danh_muc'] . '">Xóa</a></td>';
            echo '</tr>';
